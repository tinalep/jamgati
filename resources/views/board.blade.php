...
<?php
$user = Auth::user();
$content = (isset($_GET['content'])?$_GET['content']:'tab');
?>
...
<div class="dashboard">
    <div class="dashboard_header">
        ...
        <form method="get" action="{{ route('choice') }}">
            <div class="dropdown">
                <button aria-label="New" class="button button-bgorange button-no-border button-round" type="button"
    ...
    <nav class="dashboard_nav">
        <a class="dashboard_nav__link {{$content=='tab'?'dashboard_nav__link--active':''}}"
            href="?content=tab">Tableau</a>
        <a class="dashboard_nav__link {{$content=='form'?'dashboard_nav__link--active':''}}"
            href="?content=form">Formulaire</a>
    ...
    <table class="table_dashboard" style="width:100%">
        ...
        @else
        <div class="dashboard_empty">
            <p>Vous n'avez pas encore créé de tableau. <a href="{{route('table.create')}}">Commencez</a></p>
        </div>
        @endif
        ...
        <tbody>
            ...
            <tr>
                ...
                <td data-label="Dernière modification" class="table_dashboard__update">
                    {{date('d/m/y', strtotime($form->updated_at )) }}</td>
                <td data-label="Actions" class="table_dashboard__action">
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="{{'exampleModal'.$form->id}}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('form.destroy', ['form'=>$form])}}" method="post">
                                        ...
                                        <button aria-label="Submit" type="submit"
                                            class="button button-bgred button-no-border">Supprimer</button>
            ...
            <tr>
                ...
                <td data-label="Dernière modification" class="table_dashboard__update">
                    {{date('d/m/y', strtotime($nav->updated_at )) }}</td>
                <td data-label="Actions" class="table_dashboard__action">
                    {{-- <a href="#"><em class="fas fa-download"></em></a> --}}
                    ...
                    <!-- Modal -->
                    <div class="modal fade" id="{{'exampleModal'.$nav->id}}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
            ...
            <tr>
                <td scope="row" data-label="Titre" class="table_dashboard__title">{{ $table->name }}</td>
                <td data-label="Dernière modification" class="table_dashboard__update">
                    {{date('d/m/y', strtotime($table->updated_at )) }}</td>
                <td data-label="Actions" class="table_dashboard__action">
                    {{-- <a href="#"><em class="fas fa-download"></em></a> --}}
                    <a href="{{ route('table.edit', ['table'=>$table]) }}"><em class="fas fa-edit"></em></a>
                    <!-- Button trigger modal -->
                    <a type="button" data-toggle="modal" data-target="{{'#exampleModal'.$table->id}}">
                        <em class="fas fa-trash"></em>
                    </a>

                    <!-- Modal -->
                    <div class="modal fade" id="{{'exampleModal'.$table->id}}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
